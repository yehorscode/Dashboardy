import { Sun, Cloud, Moon, CloudDrizzle, CloudFog, CloudLightning, CloudMoon, CloudMoonRain, CloudRain, CloudRainWind, CloudSnow, CloudSun, CloudSunRain, Cloudy, Droplet, Droplets, Sunrise, Sunset, Thermometer, Tornado, Wind, Rainbow, Star } from "lucide-react";
...
export default function WeatherBlock() {
  const [location, setLocation] = useState<string>(localStorage.getItem("newTabWeatherLocation") || "please set location");
  ...
  async function fetchWeatherData(lat: string, lon: string) {
    try {
      ...
      const weatherData: WeatherData = {
        ...
        weatherDescription: weatherCodeToDescription[data.current.weather_code] || "Unknown",
        humidity: data.current.relative_humidity_2m,
        windSpeed: data.current.wind_speed_10m,
        sunrise: data.daily.sunrise[0],
        sunset: data.daily.sunset[0],
        icon: weatherCodeToIcon[data.current.weather_code] || <Cloud size={80} />,
      };
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold">Weather in {JSON.parse(location).name.split(",")[0]}</h1>
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p>
            <Sunrise size={16} className="inline" /> Sunrise: {new Date(weather.sunrise).toLocaleTimeString()}
          </p>
          <p>
            <Sunset size={16} className="inline" /> Sunset: {new Date(weather.sunset).toLocaleTimeString()}
          </p>
        </div>
      </div>
    </div>
